...
class F1Theme : SpeedTheme {

    override val uiColors = UiColors(
        bg      = androidx.compose.ui.graphics.Color(0xFF0A0A0A),
        primary = androidx.compose.ui.graphics.Color(0xFFFFFFFF),
        accent  = androidx.compose.ui.graphics.Color(0xFFE8002D),
        dim     = androidx.compose.ui.graphics.Color(0xFF666666),
    )

    private val bg = 0xFF0A0A0A.toInt()
    private val red = 0xFFE8002D.toInt()
    private val orange = 0xFFFF6B00.toInt()
    private val white = 0xFFFFFFFF.toInt()
    private val dim = 0xFF666666.toInt()
    private val faint = 0xFF1E1E1E.toInt()
    ...
    private fun drawTrackArc(canvas: Canvas, cx: Float, cy: Float, r: Float) {
        ...
        trackPaint.strokeWidth = r * 0.028f
    ...
    private fun drawTicks(canvas: Canvas, cx: Float, cy: Float, r: Float, maxKmh: Float) {
        ...
        while (kmh <= maxKmh + 0.01f) {
            val isMajor = kmh % 40f == 0f
            val innerR = outerR - if (isMajor) r * 0.052f else r * 0.026f
            val frac = kmh / maxKmh
            val rad = Math.toRadians((arcStartDeg + frac * arcSweepDeg).toDouble())
            tickPaint.strokeWidth = if (isMajor) r * 0.014f else r * 0.006f
            tickPaint.color = if (isMajor) white else dim
            canvas.drawLine(
    ...
    private fun drawCompass(canvas: Canvas, w: Int, h: Int, headingDeg: Float?) {
        ...
        val cardinalSize = zoneH * 0.38f
        val interSize = zoneH * 0.27f
        val numericSize = zoneH * 0.26f
        val tickCardinal = zoneH * 0.14f
        val tickInter = zoneH * 0.10f
        val tickMinorH = zoneH * 0.06f
        val labelBaseline = stripBottom - zoneH * 0.04f
        val numericBaseline = zoneH * 0.95f
        val centerX = w / 2f
        val visibleSpan = 130f
        val pxPerDeg = w / visibleSpan

        compassLinePaint.color = faint
        compassLinePaint.strokeWidth = 1f
        ...
        compassLinePaint.color = red
        compassLinePaint.strokeWidth = zoneH * 0.04f
        canvas.drawLine(centerX, 0f, centerX, stripBottom + tickCardinal + zoneH * 0.05f, compassLinePaint)
        compassLinePaint.strokeWidth = 1f

        compassLabelPaint.textSize = numericSize
        compassLabelPaint.color = red
    ...
    private fun drawGforce(canvas: Canvas, w: Int, h: Int, gx: Float, gy: Float) {
        ...
        labelPaint.textSize = r * 0.32f
        canvas.drawText(String.format(Locale.US, "%.2f G", mag), cx, cy + r + labelPaint.textSize + 6f, labelPaint)
    }
    ...
    private fun drawAccelBar(canvas: Canvas, w: Int, h: Int, accelKmhPerSec: Float) {
        ...
        canvas.drawLine(cx - barW * 0.6f, cy, cx + barW * 0.6f, cy, rectPaint)

        val ratio = (accelKmhPerSec / 12f).coerceIn(-1f, 1f)
        if (abs(ratio) > 0.02f) {
            rectPaint.style = Paint.Style.FILL
            rectPaint.color = if (ratio > 0) white else red
    ...
    private fun drawStats(canvas: Canvas, w: Int, h: Int, render: RenderState) {
        ...
        val items = listOf(
            ...
            Triple("KM", fmtKm(render.tripDistanceKm), red),
        )
        if (w > h) {
            val r = min(w, h) * 0.10f
            val colX = r + 32f
    ...
    private fun drawStatus(canvas: Canvas, cx: Float, unitY: Float, r: Float, render: RenderState) {
        ...
        val (text, color) = if (!render.locationAvailable) "● LOCATION OFF" to red else "● ACQUIRING GPS" to dim
